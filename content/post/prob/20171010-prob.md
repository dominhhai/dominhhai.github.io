...
\\cr
\ &- \ldots + (-1)^{n-1}P\Big(\prod\_{j=1}^iA_j\Big)
\end{aligned}
$$

Hay viết gọn thành:

$$ P\Bigg(\sum\_{i=1}^n{A_i}\Bigg) = \sum\_{i=1}^n (-1)^{i-1} \sum\_{k_1 \le \cdots \le k_2} P\Big(\prod\_{i=1}^nA\_{k_i}\Big) $$
...
* $\displaystyle P\Bigg(\sum\_{i=1}^n{A_i|B}\Bigg) = \sum\_{i=1}^n (-1)^{i-1} \sum\_{k_1 \le \cdots \le k_2} P\Big(\prod\_{i=1}^nA\_{k_i}|B\Big) $
